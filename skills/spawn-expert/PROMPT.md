...
- **expert_name**: Which expert to spawn - MUST be in Available Experts list (required)
- **task_title**: Short title (max 200 chars, cannot be empty) (required)
- **task_description**: Actionable description (required) - must contain:
  - **Verb**: What action to perform (create, fix, analyze, review)
  - **Object**: What to act on (script, API, config, log)
  - **Success criteria**: How to know when done (runs without errors, outputs JSON, etc.)
- **context**: Additional context, references, or constraints (optional)
- **user_task_id**: Parent user task UUID (optional) - must be valid UUID format if provided
...
### 2. Create Distilled Task in Database
